...
import { generateAllSparePartSEOElements, type SparePartSEOData, type SEOElements } from '@/utils/seo';

interface UseSparePartSEOResult {
  seoElements: SEOElements | null;
  generateSEO: (partData: SparePartSEOData) => void;
  updatePageSEO: () => void;
}

export const useSparePartSEO = (): UseSparePartSEOResult => {
  const [seoElements, setSeoElements] = useState<SEOElements | null>(null);

  const generateSEO = (partData: SparePartSEOData) => {
    try {
      const elements = generateAllSparePartSEOElements(partData);
      setSeoElements(elements);
    } catch (error) {
      console.error('Error generating spare part SEO elements:', error);
      setSeoElements(null);
    }
  };
  ...
  const updatePageSEO = () => {
    ...
    // Add structured data
    let structuredDataScript = document.querySelector('script[type="application/ld+json"]#spare-part-schema');
    if (!structuredDataScript) {
      structuredDataScript = document.createElement('script');
      structuredDataScript.setAttribute('type', 'application/ld+json');
      structuredDataScript.setAttribute('id', 'spare-part-schema');
  ...
  useEffect(() => {
    ...
    // Cleanup function
    return () => {
      const partSchema = document.querySelector('script[type="application/ld+json"]#spare-part-schema');
      if (partSchema) {
        partSchema.remove();
      }
    };
