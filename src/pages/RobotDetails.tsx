...
// The auto-generated types import has been removed.
// This complete, manual interface fixes all TypeScript errors.
interface Robot {
  id: string;
  name: string | null;
  model: string | null;
  ...
  images: string[] | null;
  technical_specifications: any; // Using 'any' for flexibility, can be more specific
...
// AI Analysis Result interface (remains the same)
...
${user?.user_metadata?.full_name || 'Interested Buyer'}`;
    ...
    try {
      ...
      if (isInWatchlist) {
        ...
        if (watchlist.includes(robot.id)) {
          toast({ title: "Already in Watchlist", description: "This robot is already in your watchlist." });
          return;
        }
      ...
      toast({ title: "Failed to Update", description: "Could not update watchlist. Please try again.", variant: "destructive" });
    } finally {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background"><EnhancedHeader /><div className="container mx-auto px-4 py-8"><div className="flex items-center justify-center py-12"><Loader2 className="w-8 h-8 animate-spin mr-2" /><span>Loading robot details...</span></div></div></div>
    );
  }

  if (error || !robot) {
    return (
      <div className="min-h-screen bg-background"><EnhancedHeader /><div className="container mx-auto px-4 py-8"><div className="flex flex-col items-center justify-center py-12"><Bot className="w-16 h-16 text-muted-foreground mb-4" /><h3 className="text-lg font-semibold mb-2">Robot Not Found</h3><p className="text-muted-foreground mb-4">{error || 'The requested robot could not be found.'}</p><Button onClick={() => navigate('/robots')} variant="outline"><ArrowLeft className="w-4 h-4 mr-2" />Back to Robots</Button></div></div></div>
    );
  }
  
  const RecommendationCard = ({ item, type }: { item: any, type: string }) => {
    const icons: Record<string, React.ReactNode> = { parts: <Wrench className="w-4 h-4 mr-2 text-blue-600" />, services: <Settings className="w-4 h-4 mr-2 text-green-600" />, logistics: <Truck className="w-4 h-4 mr-2 text-orange-600" />, finance: <DollarSign className="w-4 h-4 mr-2 text-purple-600" /> };
    const colors: Record<string, string> = { parts: 'border-blue-200', services: 'border-green-200', logistics: 'border-orange-200', finance: 'border-purple-200' };
    ...
    return (
      <Card className={`${colors[type]} hover:shadow-md transition-shadow`}><CardContent className="p-4"><div className="flex justify-between items-start"><div className="space-y-1"><h5 className="font-semibold text-gray-800 flex items-center">{icons[type]} {name}</h5>{companyName && companyName !== name && (<p className="text-sm text-muted-foreground ml-6">({companyName})</p>)}{location && (<p className="text-sm text-muted-foreground flex items-center"><MapPin className="w-3 h-3 mr-1.5 ml-6" />{location}</p>)}{item.proximity !== undefined && (<Badge variant="outline" className="ml-6 mt-1">{item.proximity}% Proximity</Badge>)}</div><Button size="sm" variant="outline" className="text-gray-600 hover:text-gray-800"><ExternalLink className="w-4 h-4" /></Button></div></CardContent></Card>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <Button variant="ghost" onClick={() => navigate('/robots')} className="mb-6"><ArrowLeft className="w-4 h-4 mr-2" />Back to Robots</Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            {user && (<Card className="border-2 border-blue-200 bg-gradient-to-br from-blue-50/80 to-purple-50/80 shadow-lg"><CardHeader className="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-t-lg p-6"><CardTitle className="flex items-center text-2xl font-bold"><Brain className="w-7 h-7 mr-3" />Smart Market Intelligence</CardTitle><p className="text-blue-100 text-sm mt-1">AI-powered insights for this specific robot and your location.</p></CardHeader><CardContent className="p-6">{!aiAnalysis ? (<div className="text-center py-6"><div className="w-24 h-24 bg-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl border-4 border-blue-100"><Brain className="w-12 h-12 text-blue-500" /></div><h3 className="text-xl font-bold text-gray-800 mb-2">Unlock Your Competitive Edge</h3><p className="text-gray-600 mb-6 max-w-lg mx-auto">Get an instant, detailed analysis of market positioning, ROI, and a complete ecosystem of suppliers and services.</p><Button onClick={handleAIAnalysis} disabled={analysisLoading} size="lg" className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 rounded-lg shadow-md hover:shadow-lg transition-all">{analysisLoading ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" />Analyzing Market...</> : <>Generate AI Analysis</>}</Button></div>) : (<div className="space-y-6"><div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm"><h4 className="font-semibold text-lg mb-3 text-gray-900 flex items-center"><Lightbulb className="w-5 h-5 mr-2 text-yellow-500" />Executive Summary</h4><p className="text-gray-700 whitespace-pre-wrap leading-relaxed">{aiAnalysis.analysis}</p></div>{(aiAnalysis.actionableRecommendations.immediateActions.length > 0 || aiAnalysis.actionableRecommendations.costOptimization.length > 0 || aiAnalysis.actionableRecommendations.riskMitigation.length > 0) && (<div className="bg-green-50 p-6 rounded-lg border border-green-200"><h4 className="font-semibold text-lg mb-3 text-green-900 flex items-center"><CheckCircle className="w-5 h-5 mr-2 text-green-600" />Actionable Recommendations</h4><ul className="list-disc list-inside space-y-1 text-green-800">{aiAnalysis.actionableRecommendations.immediateActions.map((action, i) => <li key={i}>{action}</li>)}{aiAnalysis.actionableRecommendations.costOptimization.map((action, i) => <li key={i}>{action}</li>)}{aiAnalysis.actionableRecommendations.riskMitigation.map((action, i) => <li key={i}>{action}</li>)}</ul></div>)}<Tabs defaultValue="parts" className="w-full"><TabsList className="grid w-full grid-cols-2 md:grid-cols-4 bg-gray-100 p-1 rounded-lg"><TabsTrigger value="parts" className="data-[state=active]:bg-blue-100 data-[state=active]:text-blue-800 data-[state=active]:shadow-sm">Spare Parts ({aiAnalysis.marketEcosystem.spareParts.suppliers.length})</TabsTrigger><TabsTrigger value="services" className="data-[state=active]:bg-green-100 data-[state=active]:text-green-800 data-[state=active]:shadow-sm">Services ({aiAnalysis.marketEcosystem.services.providers.length})</TabsTrigger><TabsTrigger value="logistics" className="data-[state=active]:bg-orange-100 data-[state=active]:text-orange-800 data-[state=active]:shadow-sm">Logistics ({aiAnalysis.marketEcosystem.logistics.providers.length})</TabsTrigger><TabsTrigger value="finance" className="data-[state=active]:bg-purple-100 data-[state=active]:text-purple-800 data-[state=active]:shadow-sm">Finance ({aiAnalysis.marketEcosystem.finance.providers.length})</TabsTrigger></TabsList><TabsContent value="parts" className="mt-4"><div className="space-y-3">{aiAnalysis.marketEcosystem.spareParts.suppliers.length > 0 ? (aiAnalysis.marketEcosystem.spareParts.suppliers.map((part, i) => <RecommendationCard key={i} item={part} type="parts" />)) : <p className="text-muted-foreground text-center py-4">No matching spare parts found.</p>}</div></TabsContent><TabsContent value="services" className="mt-4"><div className="space-y-3">{aiAnalysis.marketEcosystem.services.providers.length > 0 ? (aiAnalysis.marketEcosystem.services.providers.map((service, i) => <RecommendationCard key={i} item={service} type="services" />)) : <p className="text-muted-foreground text-center py-4">No matching services found.</p>}</div></TabsContent><TabsContent value="logistics" className="mt-4"><div className="space-y-3">{aiAnalysis.marketEcosystem.logistics.providers.length > 0 ? (aiAnalysis.marketEcosystem.logistics.providers.map((provider, i) => <RecommendationCard key={i} item={provider} type="logistics" />)) : <p className="text-muted-foreground text-center py-4">No matching logistics providers found.</p>}</div></TabsContent><TabsContent value="finance" className="mt-4"><div className="space-y-3">{aiAnalysis.marketEcosystem.finance.providers.length > 0 ? (aiAnalysis.marketEcosystem.finance.providers.map((provider, i) => <RecommendationCard key={i} item={provider} type="finance" />)) : <p className="text-muted-foreground text-center py-4">No matching finance providers found.</p>}</div></TabsContent></Tabs></div>)}</CardContent></Card>)}
          </div>
          <div className="space-y-6">{user && robot.profiles && (<Card><CardHeader><CardTitle className="text-xl">Seller Information</CardTitle></CardHeader><CardContent className="p-6 pt-0"><Separator className="my-4" /><div className="space-y-3 text-base"><div className="flex items-center text-gray-700"><User className="w-4 h-4 mr-3" /><span>{robot.profiles.full_name}</span></div>{robot.profiles.company_name && (<div className="flex items-center text-gray-700"><Building className="w-4 h-4 mr-3" /><span>{robot.profiles.company_name}</span></div>)}{(robot.profiles.phone || robot.profiles.mobile_number) && (<div className="flex items-center text-gray-700"><Phone className="w-4 h-4 mr-3" /><span>{robot.profiles.phone || robot.profiles.mobile_number}</span></div>)}{robot.profiles.email && (<div className="flex items-center text-gray-700"><Mail className="w-4 h-4 mr-3" /><span className="text-sm break-all">{robot.profiles.email}</span></div>)}{robot.profiles.location && (<div className="flex items-center text-gray-700"><MapPin className="w-4 h-4 mr-3" /><span>{robot.profiles.location}</span></div>)}</div></CardContent></Card>)}{!user && (<Card><CardHeader><CardTitle className="text-xl">Login Required</CardTitle></CardHeader><CardContent className="p-6 pt-0"><Separator className="my-4" /><p className="text-muted-foreground mb-4">Please log in to view seller information and access advanced AI analysis features.</p><Button onClick={() => navigate('/auth')} className="w-full">Login / Sign Up</Button></CardContent></Card>)}{user && (<Card><CardContent className="p-6 space-y-3"><Button className="w-full bg-green-600 hover:bg-green-700 text-white shadow-md" size="lg" onClick={handleContactSeller} disabled={!robot.profiles?.phone && !robot.profiles?.mobile_number}><PhoneCall className="w-5 h-5 mr-2" />Contact Seller</Button><Button variant="outline" className="w-full border-blue-600 text-blue-600 hover:bg-blue-50 shadow-md" onClick={handleRequestQuote} disabled={!robot.profiles?.email}><MessageCircle className="w-5 h-5 mr-2" />Request Quote</Button><Button variant="outline" className="w-full border-red-600 text-red-600 hover:bg-red-50 shadow-md" onClick={handleAddToWatchlist} disabled={addingToWatchlist}>{addingToWatchlist ? (<Loader2 className="w-5 h-5 mr-2 animate-spin" />) : (<Heart className={`w-5 h-5 mr-2 ${isInWatchlist ? 'fill-current text-red-500' : ''}`} />)}{isInWatchlist ? 'In Watchlist' : 'Add to Watchlist'}</Button></CardContent></Card>)}</div>
        </div>
      </div>
