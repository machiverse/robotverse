    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="container mx-auto px-4 py-8 flex-1">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-foreground">Auction Details</h2>
        ...
        {viewMode === "grid" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <CardContent className="p-0">
                  {auction.robots ? (
                    <div className="divide-y divide-border">
                      {[
                        ["Manufacturer", auction.robots.brand || "-"],
                        ["Model", auction.robots.name || "-"],
                        ["Type", auction.robots.robot_type || "-"],
                        ["Condition", <span className="capitalize">{auction.robots.condition || "Used"}</span>],
                        ["Payload", auction.robots.payload_capacity ? `${auction.robots.payload_capacity} kg` : "-"],
                        ["Reach", auction.robots.reach ? `${auction.robots.reach} mm` : "-"],
                      ].map(([label, value], idx) => (
                        <div key={idx} className="grid grid-cols-2 gap-2 p-3">
                    ...
                    <p className="text-muted-foreground p-4">Specifications unavailable.</p>
                  )}
                </CardContent>
              </Card>
            ...
            {/* Right Column: Price, Bidding, Terms */}
            <div className="space-y-6">
              {/* Price & Countdown Card */}
              <Card className="border-2 border-primary/20 shadow-md">
                <CardContent className="p-6">
                ...
                <CardContent className="p-6">
                  {canBid && !isSeller ? (
                    user ? (
                      <div className="space-y-4">
                        <div>
                          <label className="text-sm font-semibold text-foreground mb-2 block">Your bid amount</label>
                          ...
                          <p className="text-xs text-muted-foreground mt-2">Min: {formatPrice(minBid)}</p>
                        </div>
                        <Button
                          size="lg"
                        ...
                        <p className="text-xs text-muted-foreground text-center">
                          By bidding, you commit to buy if you win.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-4 text-center py-4">
                        ...
                        <Button className="h-12 text-md font-bold w-full" onClick={() => navigate("/auth")}>
                          Sign In / Register
                        </Button>
                      </div>
                    )
                    ...
                    <div className="bg-primary/10 border border-primary/20 p-4 rounded-lg text-center">
                      <CheckCircle2 className="w-8 h-8 text-primary mx-auto mb-2" />
                      <h3 className="font-semibold text-primary">Your Listing</h3>
                      <p className="text-sm text-muted-foreground mt-1">You cannot bid on your own item.</p>
                    </div>
                  ) : (
                    <p className="text-muted-foreground text-center py-4">Auction ended.</p>
                  )}
                  <AdminEditControl auctionId={id!} />
                </CardContent>
              </Card>
              ...
              <Card className="border-border">
                ...
                <CardContent className="p-4 space-y-3">
                  {auction.robots?.applications && auction.robots.applications.length > 0 && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">Applications</p>
                      <div className="flex flex-wrap gap-2">
                        {auction.robots.applications.slice(0, 4).map((a) => (
                  ...
                  {auction.warranty_period && (
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium">{auction.warranty_period}</p>
                      </div>
                    </div>
                  )}
                </CardContent>
