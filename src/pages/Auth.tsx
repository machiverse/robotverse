...
// Complete role configuration for multi-selection
  ...
  const createUserProfile = async (userId: string) => {
    try {
      ...
      const profileData: any = {
        id: userId, // FIXED: Use 'id' not 'user_id'
        full_name: fullName,
      ...
      // FIXED: Use UPSERT to handle profile creation properly
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // FIXED: Proper signup and error handling
    ...
    try {
      ...
      if (currentUser) {
        const { error } = await supabase
          ...
          .eq('id', currentUser.id); // FIXED: Use 'id' not 'user_id'
