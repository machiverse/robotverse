...
// IMPORTANT: this is your detailed full-screen view from paste.txt
...
const getMaskedValue = (value: string | null, isUnlocked: boolean): string => (isUnlocked && value ? value : "XXXXX");

const getCreditsNeeded = (leadItemType: string): number => (leadItemType === "robots" ? 10 : 5);

/* === Small subcomponents omitted here for brevity in explanation ===
   Use exactly the same StatsHeader, LeadsToolbar, ProductViewRow, LeadRow
   that were in the previous answer, NO changes needed in them.
    ...
    <div className="flex items-center justify-between gap-3">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Monitor engagement, qualify leads, and manage follow-ups in one place.
        </p>
      </div>
    </div>
...
const LeadRow = ({
  ...
  return (
    <div
      className="border-muted/60 bg-card hover:bg-accent/40 cursor-pointer rounded-md border p-4 transition-colors"
...
/* === MAIN COMPONENT integrating existing LeadDetailView === */
...
const LeadsManager = ({ sellerId, itemType }: LeadsManagerProps) => {
  ...
  const handleSendQuotationFromModal = async () => {
  ...
  return (
    <div className="space-y-4 p-6">
        ...
        <div className="p-4 pt-3">
          {viewTab === "all" ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center text-sm">
              ...
              <p className="max-w-sm text-xs text-muted-foreground">
                Leads will appear when buyers submit interest on your listings.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
      ...
      {/* Full-screen detail view re-using your existing LeadDetailView */}
      ...
      {/* Follow-up modal (light-weight, quick add) */}
      ...
      {/* Quick quotation modal (list-level) */}
      <Dialog open={showQuotationModal} onOpenChange={setShowQuotationModal}>
        <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1">
                ...
                <Textarea
                  ...
                  placeholder="Additional terms or clarifications…"
                  rows={3}
                />
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button size="sm" onClick={handleSendQuotationFromModal} disabled={sendingQuotation}>
