-- Fix the complete_user_profile function to properly handle seller roles and arrays
...
BEGIN
  ...
  RAISE LOG 'Creating/updating profile for user_id: %, account_type: %, seller_roles: %',
    p_user_id, p_account_type, p_seller_roles;

  -- Determine user roles and primary type based on account type
  -- FIX: Use cardinality() instead of array_length() for better NULL handling
  CASE p_account_type
    WHEN 'seller' THEN
  ...
  -- Check if profile already exists using fully qualified column names
  ...
  IF existing_profile_id IS NOT NULL THEN
    ...
    UPDATE public.profiles SET
      ...
      company_name = COALESCE(NULLIF(trim(p_company_name), ''), profiles.company_name),
      mobile_number = COALESCE(NULLIF(trim(p_mobile_number), ''), profiles.mobile_number),
      phone = COALESCE(NULLIF(trim(p_mobile_number), ''), profiles.phone),
      location = COALESCE(NULLIF(trim(p_location), ''), profiles.location),
      ...
      logistics_region = CASE WHEN p_account_type = 'logistics' THEN COALESCE(NULLIF(trim(p_logistics_region), ''), profiles.logistics_region) ELSE profiles.logistics_region END,
    ...
    RAISE LOG 'Profile updated successfully for user_id: %', p_user_id;
  ...
  ELSE
    ...
    RAISE LOG 'New profile created successfully for user_id: %', p_user_id;
  END IF;

  -- Return the complete profile data for verification using fully qualified column names
  RETURN QUERY
  SELECT 
    p.id as profile_id,
    p.user_id,
