...
CREATE OR REPLACE FUNCTION public.complete_user_profile(p_user_id uuid, p_email text, p_full_name text DEFAULT NULL::text, p_company_name text DEFAULT NULL::text, p_mobile_number text DEFAULT NULL::text, p_location text DEFAULT NULL::text, p_user_type text DEFAULT 'buyer'::text, p_account_type text DEFAULT 'buyer'::text, p_seller_roles text[] DEFAULT '{}'::text[], p_logistics_type text DEFAULT NULL::text, p_logistics_region text DEFAULT NULL::text, p_transport_modes text[] DEFAULT '{}'::text[], p_warehouse_storage boolean DEFAULT false, p_finance_type text[] DEFAULT '{}'::text[], p_financing_for text[] DEFAULT '{}'::text[], p_target_audience text[] DEFAULT '{}'::text[], p_government_scheme_support boolean DEFAULT false)
 RETURNS TABLE(profile_id uuid, user_id uuid, email text, full_name text, company_name text, mobile_number text, location text, account_type text, user_roles text[], registration_complete boolean, created_at timestamp with time zone, updated_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
