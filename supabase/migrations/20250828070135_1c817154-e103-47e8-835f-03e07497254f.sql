...
BEGIN
  -- Start transaction (function automatically creates a transaction)
  ...
  IF existing_profile_id IS NOT NULL THEN
    ...
    RAISE LOG 'Updating existing profile with ID: %', existing_profile_id;
    ...
    UPDATE public.profiles SET
      ...
      phone = COALESCE(NULLIF(trim(p_mobile_number), ''), mobile_number),
      ...
      logistics_region = CASE WHEN p_account_type = 'logistics' THEN NULLIF(trim(p_logistics_region), '') ELSE logistics_region END,
      ...
      warehouse_storage = CASE WHEN p_account_type = 'logistics' THEN COALESCE(p_warehouse_storage, false) ELSE COALESCE(warehouse_storage, false) END,
      ...
      government_scheme_support = CASE WHEN p_account_type = 'finance' THEN COALESCE(p_government_scheme_support, false) ELSE COALESCE(government_scheme_support, false) END,
  ...
  ELSE
    ...
    INSERT INTO public.profiles (
      ...
      COALESCE(NULLIF(trim(p_mobile_number), ''), ''),
      COALESCE(NULLIF(trim(p_mobile_number), ''), ''),
      ...
      CASE WHEN p_account_type = 'logistics' THEN COALESCE(p_warehouse_storage, false) ELSE false END,
      ...
      CASE WHEN p_account_type = 'finance' THEN COALESCE(p_government_scheme_support, false) ELSE false END,
    ...
    existing_profile_id := (SELECT id FROM public.profiles WHERE user_id = p_user_id);
  END IF;
