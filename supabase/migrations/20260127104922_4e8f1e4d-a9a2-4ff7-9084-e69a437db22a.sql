...
BEGIN
  ...
  -- Determine credits required based on item type
  -- Robots = 10 credits, Spare Parts = 5 credits, Services = 5 credits
  CASE COALESCE(v_request.item_type, 'other')
    WHEN 'robot' THEN v_credits_required := 10;
    WHEN 'robots' THEN v_credits_required := 10;
    WHEN 'spare_part' THEN v_credits_required := 5;
    WHEN 'spare_parts' THEN v_credits_required := 5;
    WHEN 'service' THEN v_credits_required := 5;
    WHEN 'services' THEN v_credits_required := 5;
    ELSE v_credits_required := 10;
  END CASE;

  -- If already converted, return the existing lead id
  IF EXISTS (
    SELECT 1
